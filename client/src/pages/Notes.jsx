...
export default function Notes() {
  const { data: notes, loading, error, createItem: createNote, updateItem: updateNote, deleteItem: removeNote } = useApi('/api/notes');
  const { data: foldersData, createItem: createFolderApi, deleteItem: deleteFolderApi } = useApi('/api/folders');
  ...
  const handleDeleteFolder = (folderId) => {
    ...
    if (folderToDelete) {
      // Note: This would need to be handled properly with the API
  ...
  const toggleFavorite = async (id) => {
    ...
    if (note) {
      try {
        await updateNote(id, { ...note, isFavorite: !note.isFavorite });
      } catch (error) {
                      ...
                      {filteredNotes.map((note) => (
                        ...
                        >
                          ...
                          <div className="mt-2 flex justify-between items-center">
                            <span className="text-xs text-gray-500 dark:text-gray-400">{formatDate(note.updated_at)}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <Card className="h-full flex flex-col">
                  <CardContent className="p-4 flex-grow flex flex-col">
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400 mb-4">
                      Last updated on {formatDate(activeNote.updated_at)}
                    </div>
